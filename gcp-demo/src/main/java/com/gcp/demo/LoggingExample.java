 ...
 *
 * <p>
 * The first parameter is an optional {@code project_id} (logged-in project will
 * be used if not supplied). Second parameter is a Logging operation and can be
 * used to demonstrate its usage. For operations that apply to more than one
 * entity (`list`, `create`, `info` and `delete`) the third parameter specifies
 * the entity.
 *//*
	...
	*//**
	...
	private static class CreateSinkAction extends LoggingAction<SinkInfo> {
		...
		public void run(Logging logging, SinkInfo sink) {
			System.out.printf("Created sink %s%n", logging);
		}
...
//				return builder.build();
	...
	*//**
	 ...
	 *
	 * @see <a href=
	 *      "https://cloud.google.com/logging/docs/api/ref_v2beta1/rest/v2beta1/monitoredResourceDescriptors/list">
...
//					.addLabel("project_id", logging.getOptions().getProjectId()).build();
...
//			System.out.printf("Written entry %s%n", entryWithResource);
		...
		LogEntry parse(String... args) throws Exception {
			if (args.length >= 3) {
				...
				return LogEntry.newBuilder(StringPayload.of(message)).setLogName(logName)
